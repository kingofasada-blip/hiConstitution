...
$englishDoc = Join-Path $rootPath "part 15 original english.docx"
$englishSimpleDoc = Join-Path $rootPath "part 15 english simple explanation.docx"
$hindiDoc = Join-Path $rootPath "part 15 hindi original.docx"
$hindiSimpleDoc = Join-Path $rootPath "part 15 hindi saral.docx"

foreach ($requiredPath in @($englishDoc, $englishSimpleDoc, $hindiDoc, $hindiSimpleDoc)) {
    if (-not (Test-Path -LiteralPath $requiredPath)) {
        throw "Missing Part 15 source file: $requiredPath"
    }
}
...
foreach ($id in $orderedIds) {
    ...
    $englishText = $enRecord.text -replace '^<strong>.*?(Article\s+\d+[A-Z]?\.)', '<strong>$1'
...
$partXV = [ordered]@{
    partId = "XV"
    partTitle = "Part XV - Elections"
    partDesc = "Articles 324 to 329A, dealing with elections and the Election Commission."
...
$updated = @()
$inserted = $false
$hasExistingPartXV = @($json | Where-Object { $_.partId -eq "XV" }).Count -gt 0

foreach ($part in $json) {
    if ($part.partId -eq "XV") {
        $updated += $partXV
        $inserted = $true
    }
    else {
        $updated += $part
        # Assuming Part XIVA is the preceding part
        if ($part.partId -eq "XIVA" -and -not $inserted -and -not $hasExistingPartXV) {
            $updated += $partXV
            $inserted = $true
        }
    }
...
Write-Output "Part XV rebuilt and inserted from the four docx sources."
