...
$englishDoc = Join-Path $rootPath "part 14 original english.docx"
...
foreach ($requiredPath in @($englishDoc, $englishSimpleDoc, $hindiDoc, $hindiSimpleDoc)) {
    if (-not (Test-Path -LiteralPath $requiredPath)) {
        throw "Missing Part XIV source file: $requiredPath"
    }
}
...
$partXIV = [ordered]@{
    partId = "XIV"
    partTitle = "Part XIV - Services Under the Union and the States"
    partDesc = "Articles 308 to 323 covering Services under the Union and the States, Public Service Commissions."
...
foreach ($part in $json) {
    if ($part.partId -eq "XIV") {
        ...
        $updated += $part
        # Assuming Part XIII is the preceding part
        if ($part.partId -eq "XIII" -and -not $inserted) {
            $updated += $partXIV
            $inserted = $true
        }
    }
...
Write-Output "Part XIV rebuilt and inserted from the four docx sources."
