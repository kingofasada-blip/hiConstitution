...
$englishDoc = Join-Path $rootPath "part 13 original english.docx"
$englishSimpleDoc = Join-Path $rootPath "part 13 english simple explanation.docx"
$hindiDoc = Join-Path $rootPath "part 13 hindi original.docx"
$hindiSimpleDoc = Join-Path $rootPath "_part 13 hindi saral.docx"

foreach ($requiredPath in @($englishDoc, $englishSimpleDoc, $hindiDoc, $hindiSimpleDoc)) {
    if (-not (Test-Path -LiteralPath $requiredPath)) {
        throw "Missing Part XIII source file: $requiredPath"
    }
}
...
function Parse-EnglishSimplifiedDoc {
    ...
    foreach ($article in $OrderedArticles) {
        $titleMap[(Get-HeadingKey -Text $article.titleText)] = $article.id
    }
...
function Parse-HindiSimplifiedDoc {
    ...
    $lines = Get-DocLines -Path $Path
    $articles = [ordered]@{}
    $pattern = "^(?:$([regex]::Escape($hindiArticleWord))\s+)?(\d{3})([A-Za-z\u0915-\u0918]*)\.\s*(.*)$"

    foreach ($rawLine in $lines) {
        $line = Normalize-SourceLine -Text $rawLine
        if ($line -match $pattern) {
            $articleId = Normalize-ArticleId -Digits $matches[1] -Suffix $matches[2]
            $articles[$articleId] = $matches[3].Trim()
        }
    }
...
$englishSimplified = Parse-EnglishSimplifiedDoc -Path $englishSimpleDoc -OrderedArticles $orderedEnglishArticles
...
$partXIII = [ordered]@{
    partId = "XIII"
    partTitle = "Part XIII - Trade, Commerce and Intercourse"
    partDesc = "Articles 301 to 307 covering freedom of trade, restrictions, State taxation limits, monopolies, omitted Article 306 and the constitutional trade authority."
...
Write-Output "Part XIII rebuilt from the four docx sources."
