...
$articles = [System.Collections.ArrayList]::new()
$currentArticle = $null
$emdash = [char]0x2014
...
foreach ($line in $lines) {
    ...
    if ($line -match $pattern) {
        if ($null -ne $currentArticle) { 
            [void]$articles.Add($currentArticle)
        }
...
# The original parts are expected to be I, II, III, IV, IVA, V
# After V, we insert VI.
foreach ($p in $jsonObj) {
    if ($p.partId -notmatch "^VI$") {
        [void]$mergedArray.Add($p)
        if ($p.partId -eq "V") {
            [void]$mergedArray.Add($part6)
        }
    }
}

$mergedArray | ConvertTo-Json -Depth 10 | Set-Content -Path "data\articles.json" -Encoding UTF8
Write-Output "Successfully merged Part VI! Total parts in JSON: $($mergedArray.Count). Part VI Articles: $($articles.Count)"
